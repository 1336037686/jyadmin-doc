...
### 2.1、JyApiDocumentProperties：Api文档配置
...
### 2.3、JyIdempotentProperties：接口幂等实现配置
...
### 2.5、JyRateLimitProperties：流量控制配置
...
### 2.6、JyRuntimeLogProperties：实时运行日志

| 配置项                           | 默认值    | 作用                       |
| -------------------------------- | --------- | -------------------------- |
| jyadmin.log.runtime-log.basePath | undefined | 设置运行日志文件的基础路径 |



### 2.7、JySecurityProperties：系统鉴权配置
...
### 2.8、JyXssFilterProperties：XSS过滤器配置
